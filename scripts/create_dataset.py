...
# python ./create_datasets/create_syncMNIST.py --split=train --no_noise_parts=0
# --dataset_multiplier=10 --save_first_n_videos=20 --predefined=True
# --predefined_path=./create_datasets/predefined_path_train.pickle

PARSER = argparse.ArgumentParser(description="Create synced dataset.")

PARSER.add_argument(
    "--max_digits_in_frame",
    type=int,
    default=5,
    help="number of digits in a frame",
)
PARSER.add_argument(
    "--frame-size", type=int, default=100, help="size of the considered image"
)
PARSER.add_argument(
    "--dataset-multiplier",
    type=int,
    default=10,
    help="make dataset X times larger than MNIST",
)
PARSER.add_argument(
    "--save-first-n-videos",
    type=int,
    default=20,
    help="save as .png the individual frames of the first X videos",
)
PARSER.add_argument(
    "--sync-dist",
    type=int,
    default=-1,
    help="maximum distance between the syncronous digits",
)  # 20
PARSER.add_argument(
    "--noise-parts-size",
    type=int,
    default=8,
    help="size of the digits parts used as noise",
)
PARSER.add_argument(
    "--no-noise-parts", type=int, default=0, help="number of noise parts"
)
PARSER.add_argument(
    "--video-frames", type=int, default=10, help="number of frames in a video"
)
...
PARSER.add_argument(
    "--predefined-path",
    type=str,
    default="./create_datasets/predefined_path_train.pickle",
    help="file for predefined digits paths (moves)",
)
PARSER.add_argument(
    "--predefined",
    action="store_true",
    help="create random paths (moves) or load from predefined ones",
)
...
def move_random_ind(coords, sz, yy, move_range=10):
    for ind in range(coords.shape[0]):
        ...
        coords[ind][0] = np.clip(
            coords[ind][0] + np.random.randint(dtop, dbot, size=[1]),
            0,
            sz - yy - 1,
        )
        coords[ind][1] = np.clip(
            coords[ind][1] + np.random.randint(dleft, dright, size=[1]),
            0,
            sz - yy - 1,
        )
    return coords
...
def move_random_sincr(coords, sz, yy, move_range=10):
    ...
    for ind in range(2):
        ...
        dbot = min(
            dbot,
            np.clip((sz - yy - 1) - coords[ind][0], -move_range, move_range),
        )

        dleft = max(dleft, np.clip(0 - coords[ind][1], -move_range, move_range))
        dright = min(
            dright,
            np.clip((sz - yy - 1) - coords[ind][1], -move_range, move_range),
        )
    if dleft < dright:
        coords[:2, 1] = np.clip(
            coords[:2, 1] + np.random.randint(dleft, dright, size=[1]),
            0,
            sz - yy - 1,
        )
    if dtop < dbot:
        coords[:2, 0] = np.clip(
            coords[:2, 0] + np.random.randint(dtop, dbot, size=[1]),
            0,
            sz - yy - 1,
        )
    return coords
...
def make_video_sincron(data):
    ...
    if not FLAGS.predefined:
        ...
        random_paths["coords_digits"] = np.zeros(
            (
                ...
                mnist_dim,
                FLAGS.video_frames,
                FLAGS.max_digits_in_frame,
                2,
            )
        ).astype(np.uint8)
        random_paths["noise_coords"] = np.zeros(
            (multiplier, mnist_dim, FLAGS.video_frames, FLAGS.no_noise_parts, 2)
        ).astype(np.uint8)
        random_paths["ind_rand_part"] = np.zeros(
            (multiplier, mnist_dim, FLAGS.no_noise_parts)
        ).astype(np.uint32)
        random_paths["ind_digits"] = np.zeros(
            (multiplier, mnist_dim, FLAGS.max_digits_in_frame)
        ).astype(np.uint32)
        random_paths["set_digits"] = np.zeros(
            (multiplier, mnist_dim, FLAGS.max_digits_in_frame - 1)
        ).astype(np.uint8)
        random_paths["sincron_prob"] = np.zeros((multiplier, mnist_dim)).astype(
            np.uint8
        )
    ...
    dataset_path = (
        f"{FLAGS.dataset_path}/{FLAGS.dataset_name}_split_{FLAGS.split}_sync_dist_{FLAGS.sync_dist}_"
    ...
    videos = np.zeros((1000, FLAGS.video_frames, 64, 64)).astype(np.uint8)
    ...
    for rep in range(FLAGS.dataset_multiplier):
        for ind in range(data[FLAGS.split + "_imgs"].shape[0]):
            ...
            # current video
            video = np.zeros((FLAGS.video_frames, 64, 64)).astype(np.uint8)
            if FLAGS.predefined:
                ...
                ind_rand_part = np.random.randint(
                    data[FLAGS.split + "_imgs"].shape[0],
                    size=[FLAGS.no_noise_parts],
                )
            ...
            for frame_no in range(FLAGS.video_frames):
                ...
                if not FLAGS.predefined:
                    ...
                    random_paths["coords_digits"][
                        rep, ind, frame_no
                    ] = coords_digits
                    random_paths["noise_coords"][
                        rep, ind, frame_no
                    ] = noise_coords
                else:
                    coords_digits = random_paths["coords_digits"][
                        rep, ind, frame_no
                    ]
                    noise_coords = random_paths["noise_coords"][
                        rep, ind, frame_no
                    ]
                ...
                for i in range(FLAGS.no_noise_parts):
                    patch = frame[
                        ...
                        + FLAGS.noise_parts_size,
                        noise_coords[i, 1] : noise_coords[i, 1]
                        + FLAGS.noise_parts_size,
                    ]
                    d1 = patch.shape[0]
                    d2 = patch.shape[1]
                ...
                for i_d, ind_digit in enumerate(ind_digits):
                    ...
                    frame_patch = frame[
                        digit_c[0] : digit_c[0] + 28,
                        digit_c[1] : digit_c[1] + 28,
                    ]
                ...
                if rep == 0 and ind <= FLAGS.save_first_n_videos:
                    pylab.imshow(
                        np.array(Image.fromarray(frame).resize((64, 64))),
                        cmap="Greys_r",
                    )
                    savepath = (
                        f"/images/video{ind}_{frame_no}_"
            ...
            if ind > 0 and (ind + 1) % 1000 == 0:
                ...
                with open(dataset_path + f"/data_{batch_no}.pickle", "wb") as f:
                    save_dict = {
                        ...
                        "labels": labels,
                        "videos_digits": videos_digits.astype(np.uint8),
